              ...
              <div class="x_panel">
               ...
               @else
                  <i class="fa fa-check fa-2x" style="color: {{$l_en}};"  aria-hidden="true"></i> 
                        ...
                        <form action="{{Route('news_del')}}" method="post" onsubmit="return confirm('{{@trans('home.confirm_del')}}')" style="float: right;">
                         {{ csrf_field() }}
                          <input name="id_news" type="hidden" value="{{$val_news->id_news}}">
                           ...
                           <input name="d_logo" type="hidden" value="{{$val_news->img_name}}">
                          <button class="btn btn-danger" type="submit">Delete</button>
                        </form>
                        <form action="{{Route('news_edit')}}" method="post" style="float: right;">
                         {{ csrf_field() }}
                          <input name="id_news" type="hidden" value="{{$val_news->id_news}}">
                        ...
                        <a href="{{url('detail/'.$val_news->id_news)}}" class="btn btn-info" role="button" target="_blank" style="float: right;">View</a>
